...
import { useGetCoursesQuery, useGetActiveCoursesQuery, useGetCompletedCoursesQuery, useGetUpcomingCoursesQuery } from "@/services/courses";
...
export default function CoursesHeaderCards({
    ...
    const totalCoursesCount = useMemo(
        () => coursesData?.result?.totalCount ?? 0,
        [coursesData]
    );

    const totalActiveCoursesCount = useMemo(
        () => activeCoursesData?.result?.totalCount ?? 0,
        [activeCoursesData]
    );
    ...
    const totalUpcomingCoursesCount = useMemo(
        () => upcomingCoursesData?.result?.totalCount ?? 0,
        [upcomingCoursesData]
    );
