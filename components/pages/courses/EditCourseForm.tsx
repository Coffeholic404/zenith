...
import { useGetCourseByIdQuery, useUpdateCourseMutation, CourseTrainerStudentUpdate } from "@/services/courses";
...
const editCoursesSchema = z.object({
...
interface TrainerWithStudents {
  ...
  students: {
    ...
    studentName: string;
    studentCode: string;
    co_St_TrId?: string; // For existing participants
  }[];
}
...
export default function EditCourseForm({ courseId }: { courseId: string }) {
  ...
  const { data: students, isLoading: isLoadingStudents, isError: isErrorStudents, isSuccess: isSuccessStudents } = useGetStudentsQuery({
    pageNumber: 1,
    pageSize: 100,
  })
  const { data: types, isLoading: isLoadingTypes, isError: isErrorTypes, isSuccess: isSuccessTypes } = useGetTypesQuery({
    pageNumber: 1,
    pageSize: 100,
  })
  ...
  useEffect(() => {
    if (courseData?.isSuccess && courseData.result && !isDataLoaded) {
      ...
      const trainerMap = new Map<string, TrainerWithStudents>();

      course.participants.forEach((participant) => {
        if (!trainerMap.has(participant.trainerId)) {
          trainerMap.set(participant.trainerId, {
        ...
        trainerMap.get(participant.trainerId)!.students.push({
          studentId: participant.studentId,
          studentName: participant.studentName,
          studentCode: participant.studentCode,
          co_St_TrId: participant.co_St_TrId
        });
      });
  ...
  const handleAddTrainer = () => {
    ...
    setAddedTrainers([...addedTrainers, {
  ...
  const handleRemoveTrainer = (trainerId: string) => {
    const trainer = addedTrainers.find(t => t.trainerId === trainerId);

    // Add all student co_St_TrIds to deleted list
    if (trainer) {
      const deletedIds = trainer.students
        .filter(s => s.co_St_TrId)
        .map(s => s.co_St_TrId!);

      setDeletedCoStTrIds([...deletedCoStTrIds, ...deletedIds]);
    }
    ...
    const updatedTrainers = addedTrainers.map(t => {
      if (t.trainerId === activeTrainerId) {
        if (checked) {
          ...
          // Remove student and track deletion if it has co_St_TrId
          const studentToRemove = t.students.find(s => s.studentId === studentId);
          if (studentToRemove?.co_St_TrId) {
            setDeletedCoStTrIds([...deletedCoStTrIds, studentToRemove.co_St_TrId]);
          }
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const costtr: CourseTrainerStudentUpdate[] = [];
    addedTrainers.forEach(trainer => {
      trainer.students.forEach(student => {
        costtr.push({
          uniqueID: student.co_St_TrId, // Include uniqueID for existing participants
    ...
    const payload = {
      ...
      typeId: data.typeId,
      costtr,
      deletedCoStTrIds: deletedCoStTrIds.length > 0 ? deletedCoStTrIds : undefined
    };
    ...
    try {
      ...
      toast({
        title: "خطأ",
        description: error?.data?.errorMessages?.join(", ") || "حدث خطأ أثناء تحديث الدورة",
        variant: "destructive"
      });
    }
  });
      ...
      <div className='scroll-smooth'>
        <Card>
          <CardContent className='py-8 text-center'>
            <p className='text-deleteTxt font-vazirmatn'>حدث خطأ أثناء تحميل بيانات الدورة</p>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <form onSubmit={onSubmit} className='grid grid-cols-1 lg:grid-cols-[1fr_450px] gap-4'>
          ...
          <div className='space-y-4' id='Basic information'>
            ...
            <Card className='px-4 py-3' id='trainer'>
              <CardContent className='p-0 space-y-4'>
                <Select value={selectedTrainerId} onValueChange={(value) => {
                    ...
                    {addedTrainers.map((trainer) => (
                      ...
                      >
                        <div className=' flex items-center gap-40'>
                          ...
                          <div className=' w-14 h-8 border rounded-xl border-[#F5F4F4] flex justify-center items-center'><p className=' text-collapsTxtClr'>{trainer.students.length}</p></div>
                        </div>
                        <Button
                          type="button"
                          variant="ghost"
          ...
          <div className='space-y-4'>
            <Card className='px-4 py-3'>
              <CardHeader className="font-vazirmatn text-subtext font-light text-[16px] px-0 py-2">
                الطلاب
              </CardHeader>
              <CardContent className='p-0 space-y-2'>
                ...
                <div className='space-y-2 max-h-[500px] overflow-y-auto'>
                  {!activeTrainerId ? (
                  ...
                  ) : availableStudents.length === 0 && !addedTrainers.find(t => t.trainerId === activeTrainerId)?.students.length ? (
                    <p className='text-subtext text-sm font-vazirmatn text-center py-4'>
                      لا يوجد طلاب متاحين
                    </p>
                  ) : (
                    <>
                      ...
                      {addedTrainers.find(t => t.trainerId === activeTrainerId)?.students.map((student) => (
                        <div key={student.studentId} className='flex items-center border border-[#8870E733/20%] space-x-2 space-x-reverse  rounded-xl px-2 py-2'>
                          <Checkbox
                            id={`student-${student.studentId}`}
                            checked={true}
                            onCheckedChange={(checked) => handleStudentToggle(student.studentId, student.studentName, checked as boolean)}
                          />
                          <label
                          ...
                          >
                            ...
                            <Badge className='text-[12px] text-white bg-[#8870E7] px-3 rounded-lg font-vazirmatn'>{student.studentCode}</Badge>
                          </label>
                        </div>
                      ))}
                      ...
                      {availableStudents.map((student: any) => (
                        <div key={student.value} className='flex items-center border border-[#8870E733/20%] space-x-2 space-x-reverse  rounded-xl px-2 py-2'>
                          <Checkbox
                            id={`student-${student.value}`}
                            className=' border-[2px] border-[#A3A2AA]'
                            checked={isStudentAssignedToActiveTrainer(student.value)}
                            onCheckedChange={(checked) => handleStudentToggle(student.value, student.label, checked as boolean)}
                          />
                          <label
