...
export default function ActivityCard({ activity }: { activity: ActivityItem }) {
  ...
  const handleDeleteActivity = async (id: string) => {
    try {
      await deleteActivity({ uniqueID: activity.uniqueID }).unwrap();

      // Show success toast
      toast({
        title: 'تم بنجاح',
        description: `تم حذف الأنشطة ${courseTitle} بنجاح`,
        variant: 'default'
      });
          ...
          <div className=" flex justify-between items-start">
            <div className=" flex flex-col gap-2">
              <div className=" font-vazirmatn space-y-1">
                <p className=" text-cardTxt font-semibold">{courseTitle}</p>
                <p className=" text-[#868585] font-light text-sm">{activity.date?.substring(0, 10)}</p>
              </div>
        ...
        <AlertDialogContent className="font-vazirmatn">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-right text-subtext">
              هل أنت متأكد من حذف الأنشطة <span className="font-semibold text-cardTxt">{activity.windSpeed}</span>؟
              <br />
          ...
          <AlertDialogFooter className="flex gap-2">
            ...
            <AlertDialogAction
              onClick={() => handleDeleteActivity(activity.uniqueID || '')}
              disabled={isDeleting}
