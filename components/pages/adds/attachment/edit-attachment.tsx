...
import { useUpdateAttachmentTypeMutation } from '@/services/attachment';
import Image from 'next/image';

// Consolidated validation schema
const editAttachmentTypeSchema = z.object({
  name: z
    .string()
...
export type EditAttachmentTypeFormData = z.infer<typeof editAttachmentTypeSchema>;

interface EditAttachmentTypeModelProps {
  id: string;
  name: string;
}

export default function EditAttachmentTypeModel({ id, name }: EditAttachmentTypeModelProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [updateAttachmentType, { isLoading }] = useUpdateAttachmentTypeMutation();
  const { toast } = useToast();

  const form = useForm<EditAttachmentTypeFormData>({
    resolver: zodResolver(editAttachmentTypeSchema),
    defaultValues: {
      name: name || ''
    }
  });
  ...
  const onSubmit = async (data: EditAttachmentTypeFormData) => {
    try {
      ...
      toast({
        title: 'تم بنجاح',
        description: 'تم تعديل نوع المرفق بنجاح',
        variant: 'default'
      });
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-[425px] rtl [&>button]:hidden space-y-4" dir="rtl">
          <DialogHeader className="text-right">
            <DialogTitle className="text-right font-vazirmatn font-bold text-[17px]">تعديل نوع المرفق</DialogTitle>
          </DialogHeader>
          <Form {...form}>
