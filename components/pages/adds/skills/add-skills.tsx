...
function SkillAddModel() {
  const [isOpen, setIsOpen] = useState(false);
  const [addSkill, { isLoading, isSuccess, isError, error }] = useCreateSkillMutation();
  ...
  const onSubmit = async (data: AddSkillFormData) => {
    try {
      ...
      toast({
        title: "تم بنجاح",
        description: "تم إضافة المهارة بنجاح",
      })
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-[425px] rtl [&>button]:hidden space-y-4" dir="rtl">
          <DialogHeader className='text-right'>
            <DialogTitle className="text-right font-vazirmatn font-bold text-[17px]">أضافة مهارة</DialogTitle>
