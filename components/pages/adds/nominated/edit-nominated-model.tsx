...
import { useUpdateNominatedPartyMutation } from '@/services/nominatedParty';
import Image from 'next/image';

// Consolidated validation schema
const editNominatedPartySchema = z.object({
  name: z
    .string()
...
export type EditNominatedPartyFormData = z.infer<typeof editNominatedPartySchema>;

interface EditNominatedModelProps {
  id: string;
  name: string;
}

export default function EditNominatedModel({ id, name }: EditNominatedModelProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [updateNominatedParty, { isLoading }] = useUpdateNominatedPartyMutation();
  const { toast } = useToast();

  const form = useForm<EditNominatedPartyFormData>({
    resolver: zodResolver(editNominatedPartySchema),
    defaultValues: {
      name: name || ''
    }
  });
  ...
  const onSubmit = async (data: EditNominatedPartyFormData) => {
    try {
      await updateNominatedParty({
        id: id,
        name: data.name.trim()
      }).unwrap();
      ...
      toast({
        title: 'تم بنجاح',
        description: 'تم تعديل جهة الترشيح بنجاح',
        variant: 'default'
      });
      ...
      console.error('Update nominated party error:', error);
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-[425px] rtl [&>button]:hidden space-y-4" dir="rtl">
          <DialogHeader className="text-right">
            <DialogTitle className="text-right font-vazirmatn font-bold text-[17px]">تعديل جهة الترشيح</DialogTitle>
          </DialogHeader>
          <Form {...form}>
