...
import { useCreateTypeMutation } from '@/services/types';
...
export type AddTypeFormData = z.infer<typeof addTypeSchema>;
function CoursesTypesModel({ btnClassName }: { btnClassName?: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [addType, { isLoading, isSuccess, isError, error }] = useCreateTypeMutation();
  const { toast } = useToast();
  const onSubmit = async (data: AddTypeFormData) => {
    try {
      const response = await addType(data).unwrap();
      toast({
        title: 'تم بنجاح',
        description: 'تم إضافة نوع الدورة بنجاح'
      });
  ...
  const form = useForm<AddTypeFormData>({
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-[425px] rtl [&>button]:hidden space-y-4" dir="rtl">
          <DialogHeader className="text-right">
            <DialogTitle className="text-right font-vazirmatn font-bold text-[17px]">أضافة نوع الدورة</DialogTitle>
...
export default CoursesTypesModel;
