...
export default function EditAccidentForm({ accidentId }: { accidentId: string }) {
  ...
  } = useGetAccidentByIdQuery({ id: accidentId }, { refetchOnMountOrArgChange: true });
  ...
  useEffect(() => {
    ...
    if (
      ...
      const hasMatchingActivity =
        activitiesData.length > 0 && activitiesData.some((activity: any) => activity.value === accident.activityId);

      // Check if student data is available AND contains the student we're looking for
      const hasMatchingStudent =
        selectedActivityId === accident.activityId &&
  ...
  const hasAnyError =
    ...
    isErrorCoStTr ||
    isErrorCourses ||
    (isSuccessActivities === false && !isLoadingActivities);
    ...
    <div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className=" space-y-4">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field orientation="responsive" data-invalid={fieldState.invalid}>
                      <Label htmlFor="form-rhf-select-language" className="font-vazirmatn text-[14px] mb-2 block">
                        اسم الدورة
                      </Label>
                        ...
                        <div key={commission.value} className="flex items-center space-x-2 space-x-reverse py-2">
                          <FieldGroup data-slot="checkbox-group">
                            <Field orientation="horizontal">
                              ...
                              />
                              <FieldLabel htmlFor={`commission-${commission.value}`} className="font-normal">
                                {commission.label}
                              </FieldLabel>
                            </Field>
                          </FieldGroup>
        ...
        <div className=" flex items-center justify-end gap-4">
          ...
          <Button type="submit" disabled={isUpdating} className=" w-[225px] bg-sidebaractive text-white  text-[14px]">
