...
const AddEvualtions = ({ participants, courseId }: AddEvaluationsProps) => {
  ...
  const unevaluatedParticipants = useMemo(() => participants?.filter(p => !p.hasEvaluation) || [], [participants]);

  // Generate default values from unevaluated participants
  const defaultValues = useMemo(
    () => ({
      evaluations: unevaluatedParticipants.map(p => ({
  ...
  const onSubmit = async (data: AddEvaluationsFormValues) => {
    ...
    const validEvaluations = data.evaluations.filter(e => e.evaluation1 || e.evaluation2 || e.evaluation3 || e.note);
    ...
    try {
      ...
      if (errorData?.errors) {
        errorMessage = Object.values(errorData.errors).flat().join('، ') as string;
