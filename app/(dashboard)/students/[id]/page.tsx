  ...
  const handleDelete = async () => {
    try {
      ...
      if (response?.isSuccess === false) {
        ...
        const errorMessage = response?.errorMessages?.[0] || 'حدث خطأ أثناء حذف الطالب. يرجى المحاولة مرة أخرى.';
      ...
      const errorMessage =
        error?.data?.errorMessages?.[0] || error?.data?.message || 'حدث خطأ أثناء حذف الطالب. يرجى المحاولة مرة أخرى.';
          ...
          <div className=" student-img flex items-end gap-2">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                {student?.result?.epilepsy && <Badge className=" font-vazirmatn bg-sidebaractive">صرع</Badge>}
                {student?.result?.heartDisease && <Badge className=" font-vazirmatn bg-sidebaractive">مرض قلب</Badge>}
                {student?.result?.sugar && <Badge className=" font-vazirmatn bg-sidebaractive">سكري</Badge>}
                {student?.result?.pressure && <Badge className=" font-vazirmatn bg-sidebaractive">ضغط</Badge>}
              </div>
            </div>
          </div>
        </div>
