        ...
        <div id="student-list" className="">
          <Card className=" min-h-full">
            ...
            <CardContent className="px-2 space-y-1">
              {jumpers.length === 0 ? (
                <p className="text-center text-subtext font-vazirmatn text-sm py-4">لا يوجد طلاب</p>
              ) : (
                jumpers.map((jumper, index) => {
                  // Find the matching CoStTr record to get student name for this jumper
                  const coStTrRecord =
                    isSuccessCoStTr && coStTr?.result?.data
                  ...
                  return (
                    ...
                    >
                      <p className="font-vazirmatn font-normal text-[16px] text-[#7B7B7B]">{jumperStudentName}</p>
                    </div>
                  );
                })
              )}
        ...
        <div id="trainer-list">
          <Card className=" min-h-full">
            ...
            <CardContent className="px-2">
              {!selectedJumper ? (
                <p className="text-center text-subtext font-vazirmatn text-sm py-4">اختر طالب لعرض المدربين</p>
              ) : getTrainers(selectedJumper).length === 0 ? (
                <p className="text-center text-subtext font-vazirmatn text-sm py-4">لا يوجد مدربين</p>
              ) : (
                getTrainers(selectedJumper).map((trainer, index) => (
                  <div key={index}>
                    <div className="py-2">
                      <p className="font-vazirmatn font-normal text-[16px] text-[#7B7B7B]">{trainer.name}</p>
                      {trainer.note && (
                        <p className="font-vazirmatn font-light text-[14px] text-subtext">{trainer.note}</p>
                      )}
                    </div>
                    {index < getTrainers(selectedJumper).length - 1 && <Separator className="my-2" />}
                  </div>
                ))
              )}
            </CardContent>
        ...
        <div id="basic-info">
          <Card className=" min-h-full">
            ...
            <CardContent className="px-2 space-y-3">
              {!selectedJumper ? (
                <p className="text-center text-subtext font-vazirmatn text-sm py-4">اختر طالب لعرض المعلومات</p>
              ) : (
                <>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">موقع الهبوط</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.landings}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">عدد القفزات</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.jumperCount}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">وقت السقوط الحر</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.freefallTime}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">نوع القفزة</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.typeOfJump}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">
                      ارتفاع الخروج من الطائرة
                    </p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.exitAltitude}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">ارتفاع السقوط الحر</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.freefallAltitude}</p>
                  </div>
                  <div className="flex flex-row justify-between items-center bg-mainBg p-3 py-3 rounded-lg">
                    <p className="font-vazirmatn font-normal text-[16px] text-studentCheckboxClr">ارتفاع فتح المظلة</p>
                    <p className="font-vazirmatn font-normal text-[14px]">{selectedJumper.deployAltitude}</p>
                  </div>
                </>
              )}
            </CardContent>
